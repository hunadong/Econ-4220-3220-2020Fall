...
\begin{mdframed}[backgroundcolor=yellow!20,linecolor=white]
You can apply the criteria above without any explanation. Note that
in exam one-dimension function is most common, which means you only need to 
calculate its derivatives to prove concavity.

Before you prove it, think about the intuition of the curve (function). Diminishing
...
\textbf{Hicksian demands are also called "compensated demands": given utility $\bar{u}$, when price $p_i$ changed a little, how your demand will change to reach the same utility level $\bar{u}$?}
